...
├── vite.config.js                # base: './'
├── package.json                  # 新增 vite / vitest devDependencies
...
│   └── unit/                     # Vitest 单测，镜像 core/ 结构
├── docs/superpowers/specs/       # 本设计文档
└── src/
    ├── main.js                   # 入口：初始化各步骤模块、全局拖拽事件
    ├── state.js                  # 全局 state 对象 + 常量
    ├── core/                     # ★ 纯逻辑：禁止 import DOM / localStorage / 文件 API
    │   ├── parser/
    │   │   ├── parse-csv.js          # parseCSV、isGarbled（UTF-8/GBK）
    │   │   ├── excel.js              # analyzeWorkbookSheets、analyzeSheet
    │   │   ├── table-detect.js       # smartDetectTable、isSummaryLikeRow、isHeaderContinuation、
    │   │   │                         #   mergeHeaderRows、hasIdentityField、inferGenderFromIdCard
    │   │   └── file-orchestrator.js  # handleFiles/processFile 的解析编排（多文件合并、构建 sources）
    │   ├── mapping/
    │   │   ├── column-detect.js      # detectColumnMapping
    │   │   └── amount-score.js       # getAmountColumnScore、isAmountLikeNumber
    │   ├── clean/
    │   │   └── values.js             # sanitizeAmount、cleanValue、身份证 X 形近字符归一
    │   ├── kb/
    │   │   ├── model.js              # migrateKB、mergeKnowledgeRows、knowledgeRowsFromKB、
    │   │   │                         #   applyConfigSheetRows、applyTaskListSheetRows
    │   │   ├── shangshe.js           # lookupShangShe 系列、detectSourceClientInfo、
    │   │   │                         #   detectBestShangSheMatch、signEntityToPlatform
    │   │   ├── tax-source.js         # inferTaxSourceByPlatformName、taxSourceForPlatform、uniqueTaxSources
    │   │   ├── task.js               # parseTaskString、normalizeTaskNameToWorkType、inferWorkType、
    │   │   │                         #   findTaskContent、generateConfigSheet、generateTaskListSheet
    │   │   └── batch.js              # chineseInitial、buildBatchNoFromShangShe、dedupeBatchNo
    │   ├── generate/
    │   │   ├── generate-output.js    # generateOutput 的纯转换部分（行/列/meta 计算）
    │   │   └── shangshe-fill.js      # fillShangSheInfo、fillShangSheInfoForRow、getRemarkPresetValue
    │   ├── templates/
    │   │   ├── registry.js           # TEMPLATES 注册表 + get(key) 查询 API
    │   │   ├── yidao.js              # 移步到岗
    │   │   ├── shenbianyun.js        # 身边云（含批次行/纯数字金额选项定义）
    │   │   ├── youyi.js              # 云杉（含配置表/任务清单 sheet、银行空补 0）
    │   │   └── custom.js             # 自定义（动态列映射）
    │   └── export/
    │       ├── naming.js             # getExportFileNameForTemplate、getTodayStr、sanitizeFileNamePart
    │       ├── split.js              # 拆分分组、ensureSplitBatches、allocSplitFileName、
    │       │                         #   buildSplitFilesForTemplates、rowsToCSV
    │       ├── workbook.js           # buildWorkbookForTemplate（查注册表分发）、workbookToArray
    │       └── zip.js                # crc32、dosDateTime、u16/u32、createZipBlob
    ├── io/                       # 副作用层：存储/文件系统，全部收敛在此
    │   ├── kb-storage.js             # localStorage 读写（loadKB/saveKB/clearKB）
    │   ├── kb-transfer.js            # 知识库 Excel/JSON 导入导出、样例下载（调 core/kb/model）
    │   └── save-file.js              # showSaveFilePicker + 降级下载（saveWorkbook/saveCSV/saveBlobAsFile）
    └── ui/                       # 渲染 + 事件：只调 core/io，不写业务规则
        ├── dom.js                    # escapeHTML、el()、事件委托辅助
        ├── steps/
        │   ├── kb-panel.js           # 知识库卡片
        │   ├── upload.js             # 上传区（文件选择、拖拽、累计指示）
        │   ├── mapping.js            # 第 2 步：映射确认
        │   ├── template.js           # 第 3 步：模版选择（含自定义字段编辑）
        │   └── export.js             # 第 4 步编排（薄壳）
        ├── export/                   # export.js 内部拆分
        │   ├── preview-table.js      # 预览表格渲染、分页
        │   ├── controls.js           # 平台/任务/批次/导出模式/身边云选项控件
        │   ├── remark-panel.js       # 备注列编辑、批量备注、行选择
        │   └── search-dropdown.js    # 商社/平台通用搜索下拉（参数化复用）
        └── column-filter.js          # 通用列筛选面板
```
...
每个模版文件默认导出统一形状，`registry.js` 聚合并提供查询：

```js
// core/templates/youyi.js 示意
export default {
  key: 'youyi',
  name: '云杉公司',
  buildWorkbook: async (ctx) => Workbook,
  // ctx: { headers, rows, meta, batchNo, kb, options, templateFile }
  buildFileName: (ctx) => string,
...
- 自定义模版是注册表成员之一，其动态列由 `custom.js` 内部处理

### 5.2 state 策略

- 保留单一 `state` 对象（`src/state.js` 导出，可变引用），不做响应式改造
...
1. 静态 HTML 中的（知识库卡片、上传区）：各步骤模块提供 `init()`，`main.js` 统一调用并 `addEventListener`
2. 动态生成的 HTML 字符串中的（预览表、导出控件）：改为**事件委托**——容器监听 click/change，按 `data-action` + `data-idx` 分发。消除"渲染函数必须把处理函数挂 window"的耦合，这是本次解耦的最大单点收益

### 5.4 io 适配层

`kb-storage.js` 只暴露 `loadKB / saveKB / clearKB`，签名与存储介质无关。未来接云端存储时实现同签名适配器替换，core/ui 一行不动。
...
| 阶段 | 内容 | 验证 |
|---|---|---|
| 0 脚手架 | Vite 初始化；HTML 骨架 + CSS 迁移；整段 JS 暂存 `src/legacy.js` 挂全局；样例文件生成基线导出产物存 `tests/fixtures/` | 页面行为与现状一致 |
| 1 core 抽取 | 按 parser → clean → mapping → kb → zip → split → templates 顺序机械搬移并即时补单测；legacy.js 逐步 import | 单测绿 + 行为不变 |
| 2 io 抽取 | kb-storage / kb-transfer / save-file 迁移；校验 core 无 localStorage / 文件 API 引用 | 单测绿 + 行为不变 |
| 3 UI 拆分 | 自底向上：dom.js → search-dropdown → column-filter → preview-table → remark-panel → controls → 五步骤模块；事件委托替换内联 onclick | 手动全流程走查 |
...
- ExcelJS 4.4.0 虽在 npm 有同版本，为统一处理同样本地化到 `public/vendor/`
...
- `vite.config.js` 设 `base: './'`（相对路径产物），同一份 `dist/` 适用于 GitHub Pages 仓库子路径、内网 nginx、任意静态托管
...
| 函数间隐式 window 全局互调 | 搬移时以显式 import 替换；ESLint `no-undef` 辅助发现遗漏 |
